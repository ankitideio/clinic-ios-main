...
//            "^(009665|9665|\\+9665|05|5)[013456789][0-9]{7}$"
    ...
    var isValidEgyptioanNumber: Bool {
        if self.isEmpty { return false }
        let mobileRegex = "^01[0125][0-9]{8}$"
        let mobileNumberTest = NSPredicate(format: "SELF MATCHES %@", mobileRegex)
        return mobileNumberTest.evaluate(with: self.withWesternNumbers)
    }
    
    var isValidNationalID: Bool {
        if self.isEmpty { return false }
        let nationalIDRegex = "([1-9]{1})([0-9]{2})([0-9]{2})([0-9]{2})([0-9]{2})[0-9]{3}([0-9]{1}[0-9]{1})$"
        let nationalIDTest = NSPredicate(format: "SELF MATCHES %@", nationalIDRegex)
        return nationalIDTest.evaluate(with: self.withWesternNumbers)
    }
